...
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for(a = va; a < endva; ){
    ...
    if(level == 1){
      uint64 base = SUPERPGROUNDDOWN(a);
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int xperm)
{
  ...
  for(a = oldsz; a < newsz; a += sz){
    ...
    memset(mem, 0, sz);
 #endif
    int mapped = sz == SUPERPGSIZE
      ? supermappage(pagetable, a, (uint64)mem, PTE_R|PTE_U|xperm)
      : mappages(pagetable, a, sz, (uint64)mem, PTE_R|PTE_U|xperm);
    if(mapped != 0){
      if(sz == SUPERPGSIZE)
        superfree(mem);
      else
